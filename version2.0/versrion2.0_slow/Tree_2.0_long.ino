...
void loop() {
    ...
    for (int i = 0; i < LEDS_NUMBER  ; i++) {
      if (capacitiveArray[i] > threshold or capacitiveArray[i] ==-2) {
         times[i] ++;
         times[i] = constrain(times[i], mintimes, maxtimes);
         strip.setPixelColor(i, (times[i]*G[i])*ramp1, times[i]*R[i]*ramp1, times[i]*B[i]*ramp1);    
         if (times[i] < maxtimes){
            Serial.print(0);
         }
         else if (times[i] == maxtimes){          
            strip.setPixelColor(i, times[i]*G[i]*ramp2, times[i]*R[i]*ramp2, times[i]*B[i]*ramp2);
            Serial.print(1); 
         }
         else if (times[i] > maxtimes-2){
            Serial.print(1); 
         }         
...
//      if (times[i]==0){
//        times[i] ++;
//        strip.setPixelColor(i, times[i]*colors[i][0]/ramp2, times[i]*colors[i][1]/ramp2, times[i]*colors[i][2]/ramp2);
//      }
    }
    strip.show();
    Serial.print('\n');
